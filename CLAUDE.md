# Claude Code Project Instructions — pathdiff-kotlin

## Project Overview

This is **pathdiff-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`pathdiff`](https://crates.io/crates/pathdiff).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/pathdiff/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/pathdiff>
