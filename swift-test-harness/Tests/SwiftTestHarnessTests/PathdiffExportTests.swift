...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Pathdiff.swiftmodule/`
...
//      `import Pathdiff` above would fail at compile time.
//
//   2. The static archive `libPathdiff.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Pathdiff" }` and
//      `flattenPackage = "io.github.kotlinmania.pathdiff"` configuration in
...
//      via the `PathdiffLibrary` product.
//
...
final class PathdiffExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Pathdiff swift module imported cleanly")
    }
}
